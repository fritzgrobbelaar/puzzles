...
def navigateToZ_Ghost(leftRightFull, mapDict, checkUniqueNess=False, maxIterations = 10000000):
    ...
    for g in range(maxIterations):
        ...
        for h, leftRight in enumerate(leftRightFull):
            for i, leftRight in enumerate(leftRight):
                if jumpCount % 1000000 == 0:
                    print('Jump count: ', jumpCount, round(g / maxIterations * 100, 2), '% at', datetime.now(),
        ...
        print(f'Dumping info: {g=}, {h=}, {i=}, {jumpCount=}, {keys=}')
